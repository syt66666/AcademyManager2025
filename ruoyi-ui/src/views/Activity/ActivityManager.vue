  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="活动容量" prop="activityCapacity">
        <el-input
          v-model="queryParams.activityCapacity"
          placeholder="请输入活动容量"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="报名截止时间" prop="activityDeadline">
        <el-date-picker clearable
                        v-model="queryParams.activityDeadline"
                        type="date"
                        value-format="yyyy-MM-dd"
    ...
    <el-table v-loading="loading" :data="activitiesList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="报名截止时间" align="center" prop="activityDeadline" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.activityDeadline, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除【请填写功能名称】编号为"' + activityIds + '"的数据项？').then(function() {
